...
class GithubSourceConfigSpec extends AnyWordSpec with Matchers {

  "GithubConfig" should {

    "parse required config" in {
      val map = Map(
    ...
    "batch size test if batch size is bigger than 250" in {
      val map = Map(
        GithubSourceConfig.TOPIC_CONFIG -> "topic-github",
        GithubSourceConfig.GITHUB_OWNER_CONFIG -> "rayyildiz",
        GithubSourceConfig.GITHUB_REPOSITORY_CONFIG -> "dotfiles",
    ...
    "batch size test if batch size is less than 0" in {
      val map = Map(
        GithubSourceConfig.TOPIC_CONFIG -> "topic-github",
        GithubSourceConfig.GITHUB_OWNER_CONFIG -> "rayyildiz",
        GithubSourceConfig.GITHUB_REPOSITORY_CONFIG -> "dotfiles",
    ...
    "batch size test is not a text" in {
      val map = Map(
        GithubSourceConfig.TOPIC_CONFIG -> "topic-github",
        GithubSourceConfig.GITHUB_OWNER_CONFIG -> "rayyildiz",
        GithubSourceConfig.GITHUB_REPOSITORY_CONFIG -> "dotfiles",
